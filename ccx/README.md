...
It is only for the `ccx` function: the console's own shell integration
(prompt marks, silent cd) is carried by ccx itself, so a bare `ccx.py` or
`ccx` binary in a fresh home — a container, a second user — still runs
commands full-screen and moves the shell silently.
